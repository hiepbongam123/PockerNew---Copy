...
namespace LoRClone.View
{
    ...
    public class DeckBuilderView : MonoBehaviour
    {
        ...
        [Header("Luật deck")]
        [Min(1)] public int maxCopies = 3;
        [Min(1)] public int minCards = 20;
        [Min(1)] public int maxCards = 40;
        ...
        /// <summary>Gọi khi đóng Xưởng Deck (dùng để màn Sảnh Đấu nạp lại lưới deck sau khi sửa).</summary>
        ...
        void BuildUI()
        {
            ...
            var overlayRT = MakeRect("DeckBuilderOverlay", parentRT, Vector2.zero, Vector2.one);
            _overlay = overlayRT.gameObject;
            var cv = _overlay.AddComponent<Canvas>();
            cv.overrideSorting = true;
            cv.sortingOrder = 600; // trên main menu (400) và lobby
            ...
            _countLabel = SpawnLabel(deckHdr, "0/40", headerFontSize * 1.25f, accentColor,
